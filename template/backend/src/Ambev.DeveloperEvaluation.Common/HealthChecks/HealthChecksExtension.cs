...
public static class HealthChecksExtension
{
    ...
    public static void AddBasicHealthChecks(this WebApplicationBuilder builder)
    {
        ...
        builder.Services.AddHealthChecks().Add(new HealthCheckRegistration(
            name: "MongoDB",
            factory: _ => new MongoDbHealthCheck(
    ...
    private static HealthCheckOptions WriteHealtCheckRespose(this WebApplication app, string tag)
    {
        ...
        {
            Predicate = string.IsNullOrEmpty(tag)
                ? _ => true
            ...
            ResponseWriter = async (context, report) =>
            {
                var result = new
                {
                    ...
                    checks = report.Entries.Select(e => new
                    {
                        ...
                        duration = $"{e.Value.Duration.TotalMilliseconds:0.00}ms",
                        description = e.Value.Description,
                        error = e.Value.Exception?.Message,
                        data = e.Value.Data
                    }),
                    totalDuration = $"{report.TotalDuration.TotalMilliseconds:0.00}ms"
                };
